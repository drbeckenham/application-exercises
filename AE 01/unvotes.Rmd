...
**Answer:** The issue categories are: Human rights, Economic Development, Colonialism, Palestinian conflict, Arms contrl and disarmament, Nuclear weapns and nuclear material
...
The United Kingdom has a very "mid" voting pattern. They have a fairly stable voting pattern or 25% Yes votes, with an increase taking place between 2000 and around 2018 to 50%.They then begin to show a decrease around 2020, dropping fairly rapidly below 50%. 

**Task 2.2:** Which country appears most consistent in its voting patterns across all issues? Which appears least consistent?

**Your answer:**

The UK seems to be the most consistent voting pattern, with the exception of votes for the Palestinian conflict. The most inconsistent country would be a toss up between the US and Turkey. Because of the wild swings, I would say Turkey is the most inconsistent. 
...
Most similar: They have very similar patterns when it comes to Nuclear weapons.

Most different: They are most different when it comes to the Palestine conflict.
...
unvotes %>%
  filter(country %in% c("United States", "Nigeria", "Mexico")) %>%  # Change these!
  mutate(year = year(date)) %>%
  group_by(country, year, issue) %>%
  summarize(percent_yes = mean(vote == "yes")) %>%
  ggplot(mapping = aes(x = year, y = percent_yes, color = country)) +
...
**Which countries did you choose?** United States, Nigeria, and Mexico

**Task 3.2:** Describe one interesting pattern you notice in your new plot.

**Your answer:**

For the most part, Mexico and Nigeria had similar patterns and values in their votes, while the US was consistently lower in their voting yes. In fact, the US often voted very differently and "anti" to how Nigeria and Mexico voted.
...
unvotes %>%
  filter(country %in% c("United Kingdom", "Australia", "South Korea", "Austria", "Italy")) %>%  # Choose 5 countries
...
It is interesting that Italy, Australia, the United Kingdom, and Austria all have similar voting patterns. Both Ital and South Korea seem to have entered the game late, with data only being collected from South Korea around the 1990s and Italy around the late 1950s. Once firmly embedded in the UN, both countries fell into line with the others, ad have similar voting patterns.
